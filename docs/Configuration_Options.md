...
* `./hokusai/config.yml` contains project-specific configuration variables.  It is loaded a top-level hash that accepts the following keys:
    ...
    - `app-version-var-name`: <string> (optional) - Store the deployed app-version in the specified env var, when invoking the `hokusai [staging|production] deploy` or the `hokusai pipeline promote` commands. Defaults to `DD_VERSION` which is what Datadog parses by default for discovering an app's version. The value stored in the var is the digest of the Docker image that was deployed.
