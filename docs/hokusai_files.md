...
- `run-template`: <string> (optional) - `hokusai [staging|production|review_app] run` creates a pod to run the user-specified command. This config specifies name of the Deployment in `hokusai/staging.yml` or `hokusai/production.yml` to model the run-pod's spec after.
...
- `secrets-file`: <string> (optional): A project may rely on secret app configs that are loaded from a file into the shell environment of its app container. This config specifies the path to that file. The content of the file must be in `export KEY='VALUE'` form. `hokusai [staging|production|review_app] run` reads this config and takes care of `source`ing the file, in addition to running the user-specified command.
